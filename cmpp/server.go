...
func (c *conn) readPacket() (*Response, error) {
	...
	case *codec.CmppActiveTestReq:
		...
		rsp = &Response{
			Packet: pkt,
			Packer: &codec.CmppActiveTestRsp{
				SeqId: p.SeqId,
			},
			SeqId: p.SeqId,
		}
	...
	case *codec.CmppTerminateRsp:
		...
		c.server.ErrorLog.Printf("receive a cmpp terminate response from %v[%d]\n",
			c.Conn.RemoteAddr(), p.SeqId)
	default:
		return nil, fmt.Errorf("readPacket: receive unsupported packet type: %#v", p)
	}
	return rsp, nil
}
...
func (c *conn) finishPacket(r *Response) error {
	if _, ok := r.Packet.Packer.(*codec.CmppActiveTestRsp); ok {
...
func ListenAndServe(addr string, typ codec.Version, t time.Duration, n int32, logWriter io.Writer, handlers ...Handler) error {
	if addr == "" {
		return ErrEmptyServerAddr
	}
